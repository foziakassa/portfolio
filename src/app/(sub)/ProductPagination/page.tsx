...
const Service = () => {
  ...
  useEffect(() => {
    if (services && services.count) {
      setTotalPage(Math.ceil(services.count / perPage));
    }
  }, [perPage, services]);
  ...
  return (
    <div className="h-full flex px-2 mx-10 -my-14">
      <div
        id="service"
        className="flex flex-col justify-center gap-40 pt-24 border my-20 rounded-md border-teal-50 shadow-lg bg-background/40"
        style={{ overflowX: "hidden", overflowY: "hidden" }}
      >
        <h1 className="text-5xl font-bold">
          <section>
            ...
            <div className="flex flex-col justify-center gap-5 px-16">
              {services.data.map((item: ServiceData) => (
                <div key={item.name} className="flex-0 w-full justify-center"> {/* Use a unique key */}
                  <Card
                    serviceData={item}
                    onClick={() => handleServiceClick(item)}
                  />
            ...
            <div className="join my-7 ml-40">
              {Array.from({ length: totalPage }).map((_, index) => (
                <button
                  key={index}
                  className={`join-item btn mx-2 ${currentPage === index + 1 ? "btn-active" : ""} text-black bg-accent/40 hover:bg-accent/80 hover:text-white p-3 rounded`}
      ...
      <div className="flex-3 bg-background/70 p-4 mt-20 mb-20 pt-20">
        {selectedService ? (
          <div className="relative flex w-[800px] h-full flex-col rounded-xl bg-background/30 bg-clip-border text-gray-100 my-10 mx-40 hover:translate-x-1 shadow-md">
            <div className="p-6">
              <h2 className="font-bold mb-2 text-accent text-3xl">
                {selectedService.name}
              </h2>
            ...
            <div className="p-6 pt-0">
              <button
                ...
                data-ripple-light="true"
                type="button"
                className="select-none rounded-lg bg-accent py-3 px-6 text-center align-middle font-sans text-xs font-bold uppercase text-gray-900 shadow-md shadow-blue-500/20 transition-all hover:shadow-lg hover:shadow-blue-500/40 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
              >
                Detail
              </button>
            </div>
